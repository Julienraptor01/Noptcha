(async () => {
    ...
    async function w({
        ...
        for (;
            "Online" !== await f.exec("get_server_status");) await m.sleep(1e3);
        var r = {
                y: "h",
                j: "c",
                i: a,
        ...
        try {
            var i = JSON.parse(r);
            if ("error" in i) return 13 === i.error ? (await m.sleep(1e3), await w({
                task: e,
                task_img: t,
                images: a
    ...
    for (;;) {
        ...
        i && (e.debug = i.debug, i.auto_open && null !== document.querySelector("div.check") ? await t(i) : i.auto_solve && null !== document.querySelector("h2.prompt-text") && await a(i))
    }
})();