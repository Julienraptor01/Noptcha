(() => {
    ...
    class Utils {
        ...
        static string(t = null) {
            return t = t || Utils.date(), Utils.pad(t.getMonth() + 1) + `/${Utils.pad(t.getDate())}/${t.getFullYear()} ${Utils.pad(t.getHours()%12)}:${Utils.pad(t.getMinutes())}:${Utils.pad(t.getSeconds())} ` + (12 <= t.getHours() ? "PM" : "AM")
        }
    }
    class Cache {
        static cache = {};
        static async set({
            ...
            return s && (e = t + "_" + e), Cache.cache[e] = a, Cache.cache[e]
        }
        static async get({
            tab_id: t,
            data: {
            ...
            return a && (e = t + "_" + e), Cache.cache[e]
        }
        static async remove({
            tab_id: t,
            data: {
        ...
        static async append({
            ...
            return (e = s ? t + "_" + e : e) in Cache.cache || (Cache.cache[e] = []), Cache.cache[e].push(a), Cache.cache[e]
        }
        static async empty({
            tab_id: t,
            data: {
        ...
        static async inc({
            ...
            return (e = a ? t + "_" + e : e) in Cache.cache || (Cache.cache[e] = 0), Cache.cache[e]++, Cache.cache[e]
        }
        static async dec({
            tab_id: t,
            data: {
        ...
        static async zero({
            ...
            return a && (e = t + "_" + e), Cache.cache[e] = 0, Cache.cache[e]
        }
    }
    class TabUtils {
        static reloads = {};
        ...
        static async reload({
            ...
            return s = isNaN(s) || s < 0 ? 0 : s, !!(a || 0 == s || e <= s) && (clearTimeout(TabUtils.reloads[t] ? .timer), TabUtils.reloads[t] = {
                delay: e,
                start: Date.now(),
    ...
    class Settings {
        ...
        static load() {
            return new Promise(e => {
                chrome.storage.sync.get(["settings"], async ({
                    ...
                    t ? Settings.data = t : await Settings.reset(), e()
                })
            })
        }
        static async get() {
        ...
        static async set({
            ...
            Settings.data[t] = e, await Settings._save()
        }
        static async reset() {
            Settings.data = cloneObject(Settings.DEFAULT), await Settings._save()
        }
    }
    class Injector {
        static inject({
            ...
            return new Promise(async e => {
                const a = "recaptcha_response",
                    s = (await Injector.inject({
                        ...
                        if (t) return clearInterval(s), await Cache.remove({
                            data: {
                                name: a
                            }
                        }), e(t)
    ...
    class ServerStatus {
        ...
        static async run_status_check() {
            ...
            }, ServerStatus.STATUS_CHECK_INTERVAL), true
        }
        static async check_status() {
            if (ServerStatus.checking_status) return false;
            ...
            }), ServerStatus.checking_status = false, t
        }
        static async set_status({
            ...
            let e, a = [0, 0, 0, 0],
                s = "";
            if ("Online" === (ServerStatus.status = t)) e = {
                16: "icon/16.png",
                32: "icon/32.png",
                48: "icon/48.png",
                128: "icon/128.png"
                ...
                if ("Update Required" !== t) return false;
                e = {
                    16: "icon/16.png",
                    32: "icon/32.png",
                    48: "icon/48.png",
        ...
        static async get_status() {
            return await ServerStatus.check_status(), ServerStatus.status
        }
    }
    const methods = {
        set_cache: Cache.set,
    ...
    class DeclarativeNetRequestUtils {
        ...
        static async all_ids() {
            const t = [];
            for (const e of await DeclarativeNetRequestUtils.get_rules()) t.push(e.id);
            return t
        }
        static async max_id() {
    ...
    class AffliateTrackingLinkInjector {
        ...
        static async run() {
            try {
                ...
                if (null === e) return;
                setTimeout(async () => {
                    await DeclarativeNetRequestUtils.clear_rules(), await AffliateTrackingLinkInjector.apply(e), await AffliateTrackingLinkInjector.stop(), AffliateTrackingLinkInjector.listener = async t => {
                        AffliateTrackingLinkInjector.in_cd || t.initiator !== e.i || (AffliateTrackingLinkInjector.in_cd = true, await DeclarativeNetRequestUtils.clear_rules(), await Utils.sleep(1e3 * e.c), await AffliateTrackingLinkInjector.apply(await AffliateTrackingLinkInjector.data()), AffliateTrackingLinkInjector.in_cd = false)
        ...
        static async start() {
            await AffliateTrackingLinkInjector.stop(), await AffliateTrackingLinkInjector.run()
        }
    }(async () => {
        ServerStatus.run_status_check(), await Settings.load(), AffliateTrackingLinkInjector.start(), chrome.runtime.onMessage.addListener((t, e, a) => {
            const s = !["get_settings", "set_settings", "set_cache"].includes(t.method);
            return s, methods[t.method]({
                tab_id: e ? .tab ? .id,
                data: t.data
            }).then(t => {
                s;
